...
import com.colusbisd.fonninez.databinding.FragmentSesionprogramadaCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing SesionProgramadaType entity.
 * This fragment is contained in the {@link SesionProgramadaActivity}.
 */
public class SesionProgramadaCreateFragment extends InterfacedFragment<SesionProgramadaType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(SesionProgramadaCreateFragment.class);
    ...
    /** SesionProgramadaType object and it's copy: the modifications are done on the copied object. */
    private SesionProgramadaType sesionProgramadaTypeEntity;
    private SesionProgramadaType sesionProgramadaTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentSesionprogramadaCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** SesionProgramadaType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for SesionProgramada to enter values for the properties.
     ...
     *            SesionProgramadaType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.sesionProgramadaType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.sesionProgramadaType.getLocalName();
        }

        ((SesionProgramadaActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            sesionProgramadaTypeEntity = createSesionProgramadaType();
        } else {
            sesionProgramadaTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        SesionProgramadaType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (SesionProgramadaType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            sesionProgramadaTypeEntityCopy = (SesionProgramadaType) sesionProgramadaTypeEntity.copy();
            sesionProgramadaTypeEntityCopy.setEntityTag(sesionProgramadaTypeEntity.getEntityTag());
            sesionProgramadaTypeEntityCopy.setOldEntity(sesionProgramadaTypeEntity);
            sesionProgramadaTypeEntityCopy.setEditLink((sesionProgramadaTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            sesionProgramadaTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, sesionProgramadaTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isSesionProgramadaTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((SesionProgramadaActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(sesionProgramadaTypeEntityCopy);
        } else {
            viewModel.update(sesionProgramadaTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new SesionProgramadaType instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new SesionProgramadaType instance
     */
    private SesionProgramadaType createSesionProgramadaType() {
        SesionProgramadaType sesionProgramadaTypeEntity = new SesionProgramadaType(true);
        return sesionProgramadaTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<SesionProgramadaType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(sesionProgramadaTypeEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentSesionprogramadaCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setSesionProgramadaType(sesionProgramadaTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isSesionProgramadaTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_sesionprogramadatype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.sesionProgramadaType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<SesionProgramadaType> result) {
