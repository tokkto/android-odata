...
import com.colusbisd.fonninez.databinding.FragmentProgramaDetailBinding;
...
/**
 * A fragment representing a single ProgramaType detail screen.
 * This fragment is contained in an ProgramaActivity.
 */
public class ProgramaDetailFragment extends InterfacedFragment<ProgramaType> {

    /** Generated data binding class based on layout file */
    private FragmentProgramaDetailBinding binding;

    /** ProgramaType entity to be displayed */
    private ProgramaType programaTypeEntity = null;
    ...
    /** Arguments: ProgramaType for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            programaTypeEntity = entity;
            binding.setProgramaType(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<ProgramaType> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, programaTypeEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull ProgramaType programaTypeEntity) {
        if (EntityMediaResource.hasMediaResources(EntitySets.programa)) {
        ...
        } else if (programaTypeEntity.getDataValue(ProgramaType.programaDesc) != null && !programaTypeEntity.getDataValue(ProgramaType.programaDesc).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(programaTypeEntity.getDataValue(ProgramaType.programaDesc).toString().substring(0, 1));
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of ProgramaType
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(programaTypeEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(programaTypeEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(programaTypeEntity));
            ...
            setDetailImage(objectHeader, programaTypeEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentProgramaDetailBinding.inflate(inflater, container, false);
