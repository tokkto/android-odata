...
import com.colusbisd.fonninez.databinding.FragmentBusinesspartnerCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing BusinessPartnerType entity.
 * This fragment is contained in the {@link BusinessPartnerActivity}.
 */
public class BusinessPartnerCreateFragment extends InterfacedFragment<BusinessPartnerType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(BusinessPartnerCreateFragment.class);
    ...
    /** BusinessPartnerType object and it's copy: the modifications are done on the copied object. */
    private BusinessPartnerType businessPartnerTypeEntity;
    private BusinessPartnerType businessPartnerTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentBusinesspartnerCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** BusinessPartnerType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for BusinessPartner to enter values for the properties.
     ...
     *            BusinessPartnerType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.businessPartnerType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.businessPartnerType.getLocalName();
        }

        ((BusinessPartnerActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            businessPartnerTypeEntity = createBusinessPartnerType();
        } else {
            businessPartnerTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        BusinessPartnerType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (BusinessPartnerType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            businessPartnerTypeEntityCopy = (BusinessPartnerType) businessPartnerTypeEntity.copy();
            businessPartnerTypeEntityCopy.setEntityTag(businessPartnerTypeEntity.getEntityTag());
            businessPartnerTypeEntityCopy.setOldEntity(businessPartnerTypeEntity);
            businessPartnerTypeEntityCopy.setEditLink((businessPartnerTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            businessPartnerTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, businessPartnerTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isBusinessPartnerTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((BusinessPartnerActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(businessPartnerTypeEntityCopy);
        } else {
            viewModel.update(businessPartnerTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new BusinessPartnerType instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new BusinessPartnerType instance
     */
    private BusinessPartnerType createBusinessPartnerType() {
        BusinessPartnerType businessPartnerTypeEntity = new BusinessPartnerType(true);
        return businessPartnerTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<BusinessPartnerType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(businessPartnerTypeEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentBusinesspartnerCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setBusinessPartnerType(businessPartnerTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isBusinessPartnerTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_businesspartnertype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.businessPartnerType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<BusinessPartnerType> result) {
