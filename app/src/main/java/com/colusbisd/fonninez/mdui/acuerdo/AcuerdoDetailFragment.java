...
import com.colusbisd.fonninez.databinding.FragmentAcuerdoDetailBinding;
...
/**
 * A fragment representing a single AcuerdoType detail screen.
 * This fragment is contained in an AcuerdoActivity.
 */
public class AcuerdoDetailFragment extends InterfacedFragment<AcuerdoType> {

    /** Generated data binding class based on layout file */
    private FragmentAcuerdoDetailBinding binding;

    /** AcuerdoType entity to be displayed */
    private AcuerdoType acuerdoTypeEntity = null;
    ...
    /** Arguments: AcuerdoType for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            acuerdoTypeEntity = entity;
            binding.setAcuerdoType(entity);
            setupObjectHeader();
        });
    }
    ...
    public void onNavigationClickedToOperacion_Operacion(View v) {
        Intent intent = new Intent(this.currentActivity, OperacionActivity.class);
        intent.putExtra("parent", acuerdoTypeEntity);
        intent.putExtra("navigation", "Operacion");
    ...
    private void onDeleteComplete(@NonNull OperationResult<AcuerdoType> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, acuerdoTypeEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull AcuerdoType acuerdoTypeEntity) {
        if (EntityMediaResource.hasMediaResources(EntitySets.acuerdo)) {
        ...
        } else if (acuerdoTypeEntity.getDataValue(AcuerdoType.codigoAcuerdoCRM) != null && !acuerdoTypeEntity.getDataValue(AcuerdoType.codigoAcuerdoCRM).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(acuerdoTypeEntity.getDataValue(AcuerdoType.codigoAcuerdoCRM).toString().substring(0, 1));
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of AcuerdoType
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(acuerdoTypeEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(acuerdoTypeEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(acuerdoTypeEntity));
            ...
            setDetailImage(objectHeader, acuerdoTypeEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentAcuerdoDetailBinding.inflate(inflater, container, false);
