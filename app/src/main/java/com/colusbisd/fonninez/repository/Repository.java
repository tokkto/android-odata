...
import com.colusbisd.fonninez.archcomp.SingleLiveEvent;
import com.colusbisd.fonninez.mediaresource.EntityMediaResource;

import com.colusbisd.fonninez.repository.OperationResult.Operation;
...
public class Repository<T extends EntityValue> {
     ...
     * @param foninez - OData service
     * @param entitySet - the entity set this repository provides access for
     */
    public Repository(@NonNull foninez foninez, @NonNull EntitySet entitySet, @Nullable Property orderByProperty) {
        this.foninez = foninez;
        this.entitySet = entitySet;
        this.orderByProperty = orderByProperty;
        observableEntities = new MutableLiveData<>();
        if (EntityMediaResource.isV4(foninez.getMetadata().getVersionCode()) && EntityMediaResource.hasMediaResources(entitySet)) {
    ...
    public void read() {
        ...
        foninez.executeQueryAsync(dataQuery,
            result -> {
    ...
    public void read(EntityValue parent, String navigationPropertyName) {
        ...
        foninez.loadPropertyAsync(navigationProperty, parent, dataQuery,
            () -> {
    ...
    public void create(@NonNull T newEntity, @NonNull StreamBase media) {
        if (newEntity.getEntityType().isMedia()) {
            foninez.createMediaAsync(newEntity, media,
                () -> {
    ...
    public void create(@NonNull T newEntity) {
        ...
        foninez.createEntityAsync(newEntity,
            () -> {
                insertToCache(newEntity, entities);
                if (relatedEntities.size() > 0) {
                    insertToCache(newEntity, relatedEntities);
    ...
    public void update(@NonNull T updateEntity) {
        foninez.updateEntityAsync(updateEntity,
            () -> {
    ...
    public void delete(@NonNull List<T> deleteEntities) {
        ...
        foninez.applyChangesAsync(deleteChangeSet,
            () -> {
    ...
    public void downloadMedia(@NonNull T entity, Action1<byte[]> successHandler,
        ...
        foninez.downloadMediaAsync(entity,
